...
```bash
adb pull $(adb shell pm path xyz.hanoman.messenger | grep /base.apk | awk -F':' '{print $2}') ~/reproducible-signal/apk-from-google-play-store/Signal-$(adb shell dumpsys package xyz.hanoman.messenger | grep versionName | awk -F'=' '{print $2}').apk
```
...
```bash
adb shell pm path xyz.hanoman.messenger
```

This will output something like:

```bash
package:/data/app/xyz.hanoman.messenger-aWRzcGlzcG9wZA==/base.apk
```

The output will tell you where the Signal APK is located in your device. (In this example the path is `/data/app/xyz.hanoman.messenger-aWRzcGlzcG9wZA==/base.apk`)
...
```bash
adb pull \
  /data/app/xyz.hanoman.messenger-aWRzcGlzcG9wZA==/base.apk \
