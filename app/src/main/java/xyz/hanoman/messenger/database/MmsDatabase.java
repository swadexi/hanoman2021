...
import xyz.hanoman.messenger.attachments.MmsNotificationAttachment;
import xyz.hanoman.messenger.contactshare.Contact;
import xyz.hanoman.messenger.database.documents.IdentityKeyMismatch;
import xyz.hanoman.messenger.database.documents.IdentityKeyMismatchList;
import xyz.hanoman.messenger.database.documents.NetworkFailure;
import xyz.hanoman.messenger.database.documents.NetworkFailureList;
...
import xyz.hanoman.messenger.database.model.NotificationMmsMessageRecord;
import xyz.hanoman.messenger.database.model.Quote;
import xyz.hanoman.messenger.database.model.ReactionRecord;
import xyz.hanoman.messenger.database.model.SmsMessageRecord;
import xyz.hanoman.messenger.database.model.databaseprotos.BodyRangeList;
...
import xyz.hanoman.messenger.mms.MessageGroupContext;
...
import xyz.hanoman.messenger.revealable.ViewOnceExpirationInfo;
...
import static xyz.hanoman.messenger.contactshare.Contact.Avatar;
...
public class MmsDatabase extends MessageDatabase {
  ...
  public void markIncomingNotificationReceived(long threadId) {
    ...
    if (xyz.hanoman.messenger.util.Util.isDefaultSmsProvider(context)) {
      DatabaseFactory.getThreadDatabase(context).incrementUnread(threadId, 1);
    }
  ...
  public static class Reader implements MessageDatabase.Reader {
    ...
    private NotificationMmsMessageRecord getNotificationMmsMessageRecord(Cursor cursor) {
      ...
      if (!TextUtils.isEmpty(contentLocation))
        contentLocationBytes = xyz.hanoman.messenger.util.Util.toIsoBytes(contentLocation);

      if (!TextUtils.isEmpty(transactionId))
        transactionIdBytes = xyz.hanoman.messenger.util.Util.toIsoBytes(transactionId);
